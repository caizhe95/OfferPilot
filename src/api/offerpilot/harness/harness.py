...
def rules_directory() -> Path:
    """Locate mounted Harness rules both in source and in the API container."""
    current = Path(__file__).resolve().parent
    for parent in (current, *current.parents):
        candidate = parent / "harness" / "rules"
        if candidate.is_dir():
            return candidate
    return current.parent / "harness" / "rules"
...
def render_report_markdown(report: dict) -> str:
    ...
    for dim_name, dim_data in cs.get("dimensions", {}).items():
        ...
        lines.append(f"| {label} ({dim_name}) | {dim_data.get('score', 0)}/10 | {str(dim_data.get('explanation', ''))[:120]} |")
    ...
    if user_covered:
        for item in user_covered[:3]:
            if isinstance(item, dict):
                lines.append(f"- **{str(item.get('point', ''))[:120]}**：{str(item.get('explanation', ''))[:120]}（回答证据：{str(item.get('evidence', ''))[:120]}）")
            else:
                lines.append(f"- {item}")
    else:
    ...
    if user_missing:
        for item in user_missing[:3]:
            if isinstance(item, dict):
                evidence = f"（已提及：{str(item['evidence'])[:120]}）" if item.get("evidence") else ""
                lines.append(f"- **{str(item.get('point', ''))[:120]}**：{str(item.get('explanation', ''))[:120]}{evidence}")
            else:
                lines.append(f"- {item}")
    else:
    ...
    lines.extend(str(item)[:220] for item in improvements[:5])

    # Follow-ups
    followups = report.get("followups", [])
    lines.extend(["", "## 可能追问"])
    if followups:
        for i, fq in enumerate(followups[:4], 1):
            lines.append(f"{i}. {str(fq.get('question', ''))[:180]}（{str(fq.get('why', ''))[:180]}）")
    else:
        lines.append("- 暂无")
    ...
    lines.extend(["", "## 知识来源"])
    if sources:
        lines.extend(f"- {str(s)[:180]}" for s in sources[:5])
    else:
