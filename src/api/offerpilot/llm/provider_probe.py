"""Live capability probe for the configured OpenAI-compatible providers.

Run with ``python -m offerpilot.llm.provider_probe``.  The probe deliberately reports
only capability metadata: it never prints API keys, prompts, or model output.
"""
...
def _probe_plain_text(client: Any) -> ProbeResult:
    try:
        ...
        return ProbeResult("plain_text", bool(content.strip()), metadata={"non_empty": bool(content.strip())})
    except Exception as exc:
        detail, status = _safe_error(exc)
        return ProbeResult("plain_text", False, detail, status)
