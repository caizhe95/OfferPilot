...
from offerpilot.llm.llm_client import structured_json_completion
...
def save_memory(
    ...
    try:
        ...
        if trace_id:
            ...
            if active is None:
                conn.rollback()
                raise RuntimeError("Memory write rejected because the active run is no longer running")
        cursor = conn.execute(
...
def persist_completed_diagnosis(
    *,
    session_id: str,
    profile_id: str,
    trace_id: str,
    ...
    """Commit report, assistant message, Run, Trace and Session as one terminal state."""
    report_id = str(uuid.uuid4())
    now = datetime.now(timezone.utc).isoformat()
    state = {"run_kind": "diagnosis", "report_id": report_id}
    conn = get_db()
    try:
        conn.execute("BEGIN IMMEDIATE")
        active = conn.execute(
        ...
        conn.execute(
            ...
            INSERT INTO diagnosis_reports
                (id, session_id, question, answer, content_scores, voice_scores, overall_score, report_markdown, diagnosis_json, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                report_id,
        ...
        conn.execute(
            ...
            (session_id, report_markdown, now),
        )
        conn.execute(
            """
            UPDATE coach_runs
            SET status = 'completed', state_json = ?, updated_at = ?, completed_at = ?
            WHERE session_id = ? AND profile_id = ? AND trace_id = ? AND status = 'running'
            """,
            (json.dumps(state, ensure_ascii=False), now, now, session_id, profile_id, trace_id),
        )
        conn.execute("UPDATE traces SET status = 'completed', updated_at = ? WHERE id = ?", (now, trace_id))
        conn.execute(
            "UPDATE sessions SET status = 'ready', updated_at = ? WHERE id = ? AND profile_id = ? AND status = 'running'",
        ...
        conn.execute(
            ...
            (
                ...
                "report_ready",
                4,
                json.dumps({"report_id": report_id, "overall_score": overall_score, "sources": sources}, ensure_ascii=False),
                now,
            ),
        )
        conn.commit()
...
def persist_diagnosis_terminal(
    *,
    session_id: str,
    profile_id: str,
    trace_id: str,
    ...
    """Persist a failed/cancelled Diagnosis terminal state without an SSE dependency."""
    now = datetime.now(timezone.utc).isoformat()
    conn = get_db()
    try:
        conn.execute("BEGIN IMMEDIATE")
        cursor = conn.execute(
            """
            UPDATE coach_runs
            SET status = ?, state_json = ?, updated_at = ?, completed_at = ?
            WHERE session_id = ? AND profile_id = ? AND trace_id = ?
              AND status IN ('running', 'cancel_requested')
            """,
            (
                status,
                json.dumps({"run_kind": "diagnosis", "error_code": error_code}, ensure_ascii=False),
                now,
                now,
                session_id,
                profile_id,
        ...
        if cursor.rowcount != 1:
            conn.rollback()
            return False
        conn.execute("UPDATE traces SET status = ?, updated_at = ? WHERE id = ?", (status, now, trace_id))
        conn.execute(
            "UPDATE sessions SET status = 'ready', updated_at = ? WHERE id = ? AND profile_id = ? AND status = 'running'",
            (now, session_id, profile_id),
        )
        conn.execute(
            "INSERT INTO trace_events (trace_id, event_type, step_index, data, created_at) VALUES (?, ?, ?, ?, ?)",
            (trace_id, f"diagnosis_{status}", 4, json.dumps({"error_code": error_code}), now),
        )
        conn.commit()
        return True
    except Exception:
