...
def parse_markdown(filepath: Path) -> dict | None:
    ...
    exam_point_records = _extract_exam_point_records(content)
    ...
    if kind == "interview_qa" and not question:
        raise ValueError(f"Interview knowledge entry has no Q section: {filepath}")
    ...
    return {
        ...
        "exam_points": [item["label"] for item in exam_point_records],
        "exam_point_ids": [item["id"] for item in exam_point_records],
...
def _extract_exam_point_records(content: str) -> list[dict[str, str]]:
    """Parse explicit, source-controlled IDs from the 考察点 section."""
    section = _section(content, "考察点")
    if not section:
        return []
    records: list[dict[str, str]] = []
    seen: set[str] = set()
    for line in section.splitlines():
        match = re.match(r"^\s*[-*]\s+\[([a-z0-9][a-z0-9.-]{2,79})\]\s+(.+?)\s*$", line)
        if not match:
            raise ValueError("Every 考察点 bullet must use '- [stable-id] label'")
        point_id, label = match.groups()
        if point_id in seen:
            raise ValueError(f"Duplicate exam point ID: {point_id}")
        seen.add(point_id)
        records.append({"id": point_id, "label": label})
    return records
...
def parse_json_file(filepath: Path) -> list[dict]:
    ...
    for item in data:
        ...
        entries.append({
            ...
            "exam_point_ids": item.get("exam_point_ids", item.get("examPointIds", [])) or [],
