...
EVAL_CASES.extend([
    ...
    {
        "name": "harness_permission_deny_fails_session",
        "kind": "harness",
        "expect": {"permission_deny_fails_session": True},
    },
    {
...
def run_eval(name: str) -> dict:
    ...
    from app.diagnosis.diagnosis import diagnose_interview
...
def _run_retrieval_eval(case: dict) -> dict:
...
def _run_harness_eval(case: dict) -> dict:
    ...
    from app.harness.harness import HarnessRunner, check_output, validate_report_structure
    from app.permission.permission import permission_gate
    from app.session.session import create_session, transition_session, save_checkpoint, get_latest_checkpoint
    ...
    elif case["name"] == "harness_permission_deny_fails_session":
        from app.permission.permission import permission_gate as pg
        result = pg.check(session_id, "save_memory", {"session_id": session_id, "key": "x"})
        request_id = result.get("request_id", "")
        pg.deny(request_id)
        from app.session.session import fail_after_denial, get_session
        fail_after_denial(session_id, "evals_deny")
        s = get_session(session_id)
        passed = s is not None and s.get("status") == "failed"
        details["session_status"] = s.get("status") if s else None
        if not passed:
            reasons.append("Session not marked failed after deny")
