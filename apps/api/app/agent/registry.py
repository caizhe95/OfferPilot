...
from app.diagnosis.diagnosis import get_memories, list_recent_reports, save_memory
from app.diagnosis.workflow import run_diagnosis
...
class DiagnosisArgs(StrictModel):
    question: str = Field(min_length=1, max_length=1000)
...
def create_coach_registry(session_id: str, profile_id: str, trace_id: str, remaining_timeout: Any | None = None) -> ToolRegistry:
    registry = ToolRegistry()
    registry.register(_definition("search_knowledge", "Search the internal technical interview knowledge base.", SearchKnowledgeArgs, "low", lambda p: search_knowledge_safe(question=p["question"], answer=p["answer"], limit=p["limit"], trace_id=trace_id)))
    ...
    registry.register(_definition("recommend_next_question", "Recommend a question from the internal knowledge base; never invent one.", RecommendArgs, "low", lambda p: _recommend(p["focus"], trace_id)))
    registry.register(_definition("run_diagnosis", "Formally score one explicit interview question and candidate answer.", DiagnosisArgs, "low", lambda p: run_diagnosis(session_id=session_id, profile_id=profile_id, trace_id=trace_id, question=p["question"], answer=p["answer"], timeout=float(remaining_timeout() if remaining_timeout else 25.0))))
    registry.register(_definition("save_memory", "Request approval to save one coaching memory.", SaveMemoryArgs, "high", lambda p: save_memory(session_id, p["key"], p["value"], p["category"], profile_id=profile_id)))
    return registry


def _recommend(focus: str, trace_id: str) -> dict[str, Any]:
    items = search_knowledge_safe(question=focus or "技术面试练习", answer="", limit=1, trace_id=trace_id)
    if not items:
