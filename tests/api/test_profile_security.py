...
def test_profile_cannot_read_another_profiles_session():
    with TestClient(app) as owner, TestClient(app) as other:
        ...
        assert other.get(f"/api/sessions/{created.json()['id']}").status_code == 403
...
def test_legacy_bootstrap_requires_its_temporary_opt_in(client, monkeypatch):
    client.cookies.clear()
    ...
    monkeypatch.setattr(settings, "allow_legacy_profile_bootstrap", True)
    response = client.post(
        "/api/profile/bootstrap",
        json={"legacy_profile_id": PROFILE_ONE},
        headers={"Origin": "http://localhost:3000"},
    ...
    assert response.status_code == 200
    assert response.json() == {"profile_id": PROFILE_ONE, "migrated": True}
